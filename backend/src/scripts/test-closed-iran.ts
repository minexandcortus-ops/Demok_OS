...
async function bootstrap() {
    console.log('🚀 Simulation: Clôture du sondage "Guerre en Iran"...');
    ...
    try {
        ...
        // Trouver le sondage
        const poll = await pollRepo.findOne({ where: { question: Like('%Iran%') } });
        if (!poll) {
            console.error('❌ Sondage sur l\'Iran non trouvé en base de données.');
            process.exit(1);
        }
